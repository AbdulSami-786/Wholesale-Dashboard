...
export async function syncQueue(onProgress) {
  ...
  saveQueue(remaining);
  if (synced > 0) {
    for (const sheet of ["Parties","Items","Bills","Ledger","Cash","SharedLedgers","MonthlyClosing"]) {
      try {
        const data = await callNetwork("getSheet", { sheet });
        setCache(sheet, data || []);
      } catch {}
...
// ── MonthlyClosing ────────────────────────────────────────────
export function rowToMonthlyClosing([id, partyId, partyName, month, year, openingBalance, totalSales, totalPayments, totalDiscount, closingBalance, status, lastUpdated]) {
  return {
