...
//     return {
//       sno: idx + 1, id: p.id, name: p.name, city: p.city, phone: p.phone,
//       date: lastDate, openingBal: p.openingBalance, totalDebit, totalCredit,
//       totalPending: netBalance, closingBal: netBalance,
//     };
//   });
...
//     return `<!DOCTYPE html><html><head><title>Parties Pending Balance Sheet</title>
// <style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;background:#fff;color:#111;font-size:13px}
// @media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.no-print{display:none!important}table{page-break-inside:auto}tr{page-break-inside:avoid}}
...
//     <div class="sbox"><div class="sbox-label">Total Parties</div><div class="sbox-val" style="color:#0f6e56">${partyData.length}</div></div>
//     <div class="sbox"><div class="sbox-label">Outstanding Parties</div><div class="sbox-val" style="color:#dc2626">${partyData.filter(p=>p.totalPending>0).length}</div></div>
//     <div class="sbox"><div class="sbox-label">Grand Pending Balance</div><div class="sbox-val" style="color:#dc2626">Rs ${Number(grandPending).toLocaleString("en-PK")}</div></div>
//     <div class="sbox"><div class="sbox-label">Clear Accounts</div><div class="sbox-val" style="color:#16a34a">${partyData.filter(p=>p.totalPending<=0).length}</div></div>
//   </div>
//   <table>
//     <thead>
//       <tr>
...
// // ══════════════════════════════════════════════════════════════
// // QUICK PAYMENT MODAL (used from Party Master)
// // ══════════════════════════════════════════════════════════════
// function QuickPaymentModal({ party, currentBalance, onClose, onSave, saving }) {
...
//     onSave({ amount: Number(amount), date, ref: ref || uid("REC"), note });
//   }

//   return (
//     <Modal title={`💵 Record Payment — ${party.name}`} onClose={onClose} width={460}>
//       <div style={{
//         background: currentBalance > 0 ? "#FCEBEB" : "#EAF3DE",
//         border: `1px solid ${currentBalance > 0 ? "#F5C3C3" : "#C3E8C3"}`,
//         borderRadius: 10, padding: "12px 16px", marginBottom: 16,
//         display: "flex", justifyContent: "space-between", alignItems: "center",
//       }}>
//         <div>
//           <div style={{fontSize:11,color:COLORS.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.04em"}}>Current Balance</div>
//           <div style={{fontSize:20,fontWeight:800,color: currentBalance > 0 ? COLORS.danger : COLORS.primary, marginTop:2}}>{fmt(currentBalance)}</div>
//         </div>
//         <div style={{fontSize:28}}>{currentBalance > 0 ? "⏳" : "✅"}</div>
//       </div>

//       <div style={{display:"flex",flexDirection:"column",gap:12}}>
//         <Input label="Payment Amount (Rs)" type="number" value={amount} onChange={setAmount} placeholder="e.g. 5000" />
//         <Input label="Date" type="date" value={date} onChange={setDate} />
//         <Input label="Reference No (optional)" value={ref} onChange={setRef} placeholder="e.g. REC-012" />
//         <Input label="Note" value={note} onChange={setNote} placeholder="Payment received" />

//         {amount && Number(amount) > 0 && (
//           <div style={{background:"#F0FBF6",border:"1px solid #C3E8D8",borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
//             <span style={{fontSize:12,color:COLORS.muted,fontWeight:600}}>Balance after payment</span>
//             <span style={{fontSize:16,fontWeight:800,color: (currentBalance - Number(amount)) > 0 ? COLORS.danger : COLORS.primary}}>
//               {fmt(currentBalance - Number(amount))}
//             </span>
//           </div>
//         )}
...
//         <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:4}}>
//           <Btn variant="secondary" onClick={onClose}>Cancel</Btn>
//           <Btn onClick={handleSave} disabled={saving || !amount}>
...
//   if (loading) return (
//     <div style={{minHeight:"100vh",background:"#F8F9FA",display:"flex",alignItems:"center",justifyContent:"center"}}>
//       <div style={{textAlign:"center",color:COLORS.muted}}><div style={{fontSize:36,marginBottom:12}}>📒</div><div>Loading…</div></div>
//     </div>
//   );
...
//           <div style={{background:"#fff",borderRadius:12,border:`1px solid ${COLORS.border}`,padding:"1rem"}}>
//             <div style={{fontSize:11,color:COLORS.muted,textTransform:"uppercase",fontWeight:600,marginBottom:4}}>Opening Balance</div>
//             <div style={{fontSize:18,fontWeight:700}}>{fmt(data.openingBalance)}</div>
//           </div>
//           <div style={{background:"#fff",borderRadius:12,border:`1px solid ${COLORS.border}`,padding:"1rem"}}>
//             <div style={{fontSize:11,color:COLORS.muted,textTransform:"uppercase",fontWeight:600,marginBottom:4}}>Total Debit</div>
//             <div style={{fontSize:18,fontWeight:700,color:COLORS.danger}}>{fmt(totalDebit)}</div>
...
//         <div style={{display:"flex",gap:0,marginBottom:16,background:"#fff",borderRadius:10,border:`1px solid ${COLORS.border}`,overflow:"hidden",width:"fit-content"}}>
//           {[
//             {id:"ledger", label:"📒 Ledger Entries"},
//             {id:"bills",  label:`🧾 Bills (${partyBills.length})`},
//           ].map(tab=>(
...
//         {activeTab === "ledger" && (
//           <div style={{background:"#fff",borderRadius:12,border:`1px solid ${COLORS.border}`,padding:"1.25rem"}}>
//             <div style={{fontSize:16,fontWeight:700,fontFamily:"Georgia,serif",marginBottom:16}}>
//               Transaction History ({data.entries.length} entries)
//             </div>
//             <Table columns={[
...
//           <div style={{background:"#fff",borderRadius:12,border:`1px solid ${COLORS.border}`,padding:"1.25rem"}}>
//             <div style={{fontSize:16,fontWeight:700,fontFamily:"Georgia,serif",marginBottom:16}}>
//               Bills / Invoices ({partyBills.length})
//             </div>
//             {partyBills.length === 0 ? (
//               <div style={{textAlign:"center",padding:"40px 20px",color:COLORS.muted}}>
//                 <div style={{fontSize:32,marginBottom:8}}>🧾</div>
//                 <div style={{fontSize:14}}>No bills found for this party.</div>
//               </div>
//             ) : (
//               <div style={{overflowX:"auto"}}>
//                 <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
//                   <thead>
//                     <tr style={{borderBottom:`2px solid ${COLORS.border}`}}>
//                       {["Bill #","Date","Items","Type","Discount","Total",""].map((h,i)=>(
//                         <th key={i} style={{padding:"10px 12px",textAlign:i>=4?"right":i===6?"center":"left",color:COLORS.muted,fontWeight:600,fontSize:11,textTransform:"uppercase",letterSpacing:"0.05em",whiteSpace:"nowrap"}}>{h}</th>
//                       ))}
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {[...partyBills].reverse().map((bill,i)=>(
//                       <tr key={bill.id||i} style={{borderBottom:`1px solid ${COLORS.border}`,background:i%2===0?"#fff":"#FAFAFA"}}>
//                         <td style={{padding:"10px 12px",fontWeight:700,color:COLORS.text}}>{bill.billNo}</td>
//                         <td style={{padding:"10px 12px",whiteSpace:"nowrap"}}>{isoToDisplay(bill.date)}</td>
//                         <td style={{padding:"10px 12px"}}>{bill.items?.length || 0} item(s)</td>
//                         <td style={{padding:"10px 12px"}}>
//                           <Badge color={bill.type==="cash"?"green":"amber"}>{bill.type==="cash"?"Cash":"Credit"}</Badge>
//                         </td>
//                         <td style={{padding:"10px 12px",textAlign:"right"}}>
//                           {Number(bill.discount||0)>0
//                             ? <span style={{color:COLORS.danger,fontWeight:600}}>−{fmt(bill.discount)}</span>
//                             : <span style={{color:COLORS.muted}}>—</span>
//                           }
//                         </td>
//                         <td style={{padding:"10px 12px",textAlign:"right",fontWeight:700,color:COLORS.text}}>{fmt(bill.total)}</td>
//                         <td style={{padding:"10px 12px",textAlign:"center"}}>
//                           <button
//                             onClick={()=>setViewingBill(bill)}
//                             style={{
//                               background:"#E6F1FB",color:"#185FA5",border:"1px solid #C3D9F5",
...
//                   </tbody>
//                   <tfoot>
//                     <tr style={{background:"#F0FBF6",borderTop:`2px solid ${COLORS.border}`}}>
//                       <td colSpan={5} style={{padding:"10px 12px",fontWeight:700,fontSize:13,color:COLORS.text}}>Grand Total ({partyBills.length} bills)</td>
//                       <td style={{padding:"10px 12px",textAlign:"right",fontWeight:800,fontSize:15,color:COLORS.primary}}>
//                         {fmt(partyBills.reduce((s,b)=>s+b.total,0))}
//                       </td>
//                       <td></td>
//                     </tr>
//                   </tfoot>
...
//       .map(b => ({
//         id: b.id, date: b.date, billNo: b.billNo, partyId: b.partyId, partyName: b.partyName,
//         subtotal: b.items.reduce((s,i)=>s+i.total,0),
//         discount: Number(b.discount||0), discountType: b.discountType||"fixed",
//         discountVal: b.discountVal||0, grandTotal: b.total,
//       }));
//   }, [bills, start, end]);
...
//   // ── FIX: removed separate discount credit ledger entry ──────────────────
//   async function saveBill() {
//     if(!form.partyId||form.lines.some(l=>!l.itemId)) return alert("Fill all fields");
//     setSaving(true); setSyncStatus("saving");
//     try {
...
//         : Number(form.discountVal||0);
//       const total = subtotal - discountAmt; // grand total after discount
...
//         if(form.type==="credit") {
//           // Single debit entry for the grand total (discount already deducted — no separate credit entry needed)
//           const entry={id:uid("l"),partyId:form.partyId,partyName:party.name,date:form.date,type:"debit",ref:form.billNo,note:"Sale",amount:total};
//           await appendRow("Ledger", ledgerToRow(entry));
//           setLedger(prev=>[...prev,entry]);
//           // ✅ Removed: separate discount credit entry that was incorrectly reducing balance further
//         } else {
//           // Cash sale — goes to cash book
//           const entry={id:uid("c"),date:form.date,type:"in",ref:form.billNo,note:`Cash sale - ${party.name}`,amount:total};
//           await appendRow("Cash", cashToRow(entry));
//           setCash(prev=>[...prev,entry]);
//         }
...
// //  PARTY MASTER SCREEN
// // ══════════════════════════════════════════════════════════════
// function PartyMasterScreen({ parties, setParties, ledger, setLedger, bills, setSyncStatus }) {
//   const [showForm,       setShowForm]       = useState(false);
//   const [editingParty,   setEditingParty]   = useState(null);
//   const [confirmDel,     setConfirmDel]     = useState(null);
//   const [saving,         setSaving]         = useState(false);
...
//     const entries = ledger.filter(l => l.partyId === party.id);
//     const net = entries.reduce((s,e) => e.type==="debit" ? s+e.amount : s-e.amount, 0);
//     return party.openingBalance + net;
//   }

//   function openEdit(party) {
//     setEditingParty(party);
//     setForm({name:party.name,phone:party.phone,city:party.city,address:party.address,creditLimit:party.creditLimit,openingBalance:party.openingBalance});
//     setShowForm(true);
//   }
...
//   async function handleQuickPayment({ amount, date, ref, note }) {
//     if (!payParty) return;
//     setPaySaving(true); setSyncStatus("saving");
//     try {
//       const entry = {
//         id:        uid("l"),
//         partyId:   payParty.id,
...
//   const partyRows = parties.map(p => ({ ...p, _balance: getBalance(p) }));
...
//           {key:"creditLimit",    label:"Credit Limit",    right:true, render:v=>fmt(v)},
//           {key:"openingBalance", label:"Opening Bal",     right:true, render:v=>fmt(v)},
//           {key:"_balance",       label:"Current Balance", right:true,
//             render:(v)=>(
//               <span style={{fontWeight:700,fontSize:13,color:v>0?COLORS.danger:v<0?COLORS.primary:COLORS.muted}}>
//                 {fmt(v)}
//               </span>
//             )
//           },
...
//               <button
//                 onClick={()=>setPayParty(row)}
//                 style={{
//                   background:"#EAF3DE",color:"#3B6D11",border:"1px solid #C3E8C3",
...
//       {confirmDel && (
//         <ConfirmDialog message={`Delete party "${confirmDel.name}"?`} onConfirm={()=>deleteParty(confirmDel)} onCancel={()=>setConfirmDel(null)} />
//       )}
...
//         <QuickPaymentModal
//           party={payParty}
//           currentBalance={payParty._balance}
...
//       {showPrint && (
//         <PartiesPrintModal parties={parties} ledger={ledger} onClose={()=>setShowPrint(false)} />
//       )}
//     </div>
//   );
// }
...
//     stock:       <StockScreen       items={items} setItems={setItems} {...sharedProps} />,
//     reports:     <ReportsScreen     bills={bills} cash={cash} ledger={ledger} parties={parties} />,
//     parties:     <PartyMasterScreen parties={parties} setParties={setParties} ledger={ledger} setLedger={setLedger} bills={bills} {...sharedProps} />,
//   };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F8F9FA",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"24px 16px"}}>
        ...
        <div style={{display:"flex",gap:0,marginBottom:16,background:"#fff",borderRadius:10,border:`1px solid ${COLORS.border}`,overflow:"hidden",width:"fit-content"}}>
          {[{id:"ledger",label:"📒 Ledger Entries"},{id:"bills",label:`🧾 Bills (${partyBills.length})`}].map(tab=>(
            <button key={tab.id} onClick={()=>setActiveTab(tab.id)}
              style={{background:activeTab===tab.id?COLORS.primary:"transparent",color:activeTab===tab.id?"#fff":COLORS.muted,border:"none",padding:"10px 22px",fontWeight:700,fontSize:13,cursor:"pointer",transition:"all 0.15s"}}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        {activeTab === "bills" && (
          <div style={{background:"#fff",borderRadius:12,border:`1px solid ${COLORS.border}`,padding:"1.25rem"}}>
            <div style={{fontSize:16,fontWeight:700,fontFamily:"Georgia,serif",marginBottom:16}}>Bills / Invoices ({partyBills.length})</div>
            {partyBills.length === 0 ? (
              <div style={{textAlign:"center",padding:"40px 20px",color:COLORS.muted}}><div style={{fontSize:32,marginBottom:8}}>🧾</div><div style={{fontSize:14}}>No bills found for this party.</div></div>
            ) : (
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  <thead>
                  ...
                  <tbody>
                    {[...partyBills].reverse().map((bill,i)=>(
                      <tr key={bill.id||i} style={{borderBottom:`1px solid ${COLORS.border}`,background:i%2===0?"#fff":"#FAFAFA"}}>
                        ...
                        <td style={{padding:"10px 12px",whiteSpace:"nowrap"}}>{isoToDisplay(bill.date)}</td>
                        <td style={{padding:"10px 12px"}}>{bill.items?.length || 0} item(s)</td>
                        <td style={{padding:"10px 12px"}}><Badge color={bill.type==="cash"?"green":"amber"}>{bill.type==="cash"?"Cash":"Credit"}</Badge></td>
                        <td style={{padding:"10px 12px",textAlign:"right"}}>{Number(bill.discount||0)>0?<span style={{color:COLORS.danger,fontWeight:600}}>−{fmt(bill.discount)}</span>:<span style={{color:COLORS.muted}}>—</span>}</td>
                        <td style={{padding:"10px 12px",textAlign:"right",fontWeight:700,color:COLORS.text}}>{fmt(bill.total)}</td>
                        <td style={{padding:"10px 12px",textAlign:"center"}}>
                          <button onClick={()=>setViewingBill(bill)} style={{background:"#E6F1FB",color:"#185FA5",border:"1px solid #C3D9F5",borderRadius:7,padding:"5px 12px",fontSize:12,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:5}}>👁 View Bill</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr style={{background:"#F0FBF6",borderTop:`2px solid ${COLORS.border}`}}>
                      <td colSpan={5} style={{padding:"10px 12px",fontWeight:700,fontSize:13,color:COLORS.text}}>Grand Total ({partyBills.length} bills)</td>
                      <td style={{padding:"10px 12px",textAlign:"right",fontWeight:800,fontSize:15,color:COLORS.primary}}>{fmt(partyBills.reduce((s,b)=>s+b.total,0))}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                </table>
    ...
    try {
      ...
      if(editingBill) {
        ...
        const newBill = {id:uid("b"),partyId:form.partyId,partyName:party.name,date:form.date,billNo:form.billNo,
          ...
          discount:discountAmt,discountType:form.discountType,discountVal:Number(form.discountVal||0)};
        await appendRow("Bills", billToRow(newBill));
        setBills(prev=>prev.map(b=>b.id===newBill.id?b:b).concat(
          prev.find(b=>b.id===newBill.id) ? [] : [newBill]
        ));

        if(form.type==="credit") {
          const entry={id:uid("l"),partyId:form.partyId,partyName:party.name,date:form.date,type:"debit",ref:form.billNo,note:"Sale",amount:total};
          await appendRow("Ledger", ledgerToRow(entry));
          setLedger(prev=>prev.find(e=>e.id===entry.id)?prev:[...prev,entry]);
        } else {
          const entry={id:uid("c"),date:form.date,type:"in",ref:form.billNo,note:`Cash sale - ${party.name}`,amount:total};
          await appendRow("Cash", cashToRow(entry));
          setCash(prev=>prev.find(e=>e.id===entry.id)?prev:[...prev,entry]);
        }
    ...
    <div>
      ...
      {showForm && (
        <Modal title={editingBill?"Edit Bill":"New Bill"} onClose={()=>setShowForm(false)} width={680}>
          ...
          <div style={{background:"#F8F9FA",borderRadius:8,padding:"12px 14px",marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"4px 0",color:COLORS.muted}}>
              <span>Subtotal ({form.lines.length} items)</span><span>{fmt(formSubtotal)}</span>
            </div>
            {formDiscountAmt>0&&(
              <div style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"4px 0",color:COLORS.danger}}>
                <span>Discount {form.discountType==="pct"?`(${form.discountVal}%)`:"(fixed)"}</span>
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontSize:18,fontWeight:800,padding:"10px 0 0",marginTop:6,borderTop:`2px solid ${COLORS.border}`,color:COLORS.text}}>
              <span>Grand Total</span><span style={{color:COLORS.primary}}>{fmt(formGrandTotal)}</span>
            </div>
          </div>
          <Input label="Notes" value={form.notes} onChange={v=>setForm(f=>({...f,notes:v}))} placeholder="Optional note…" />
          <div style={{display:"flex",gap:8,marginTop:16,justifyContent:"flex-end"}}>
  ...
  async function saveShareLink({ slug, password, expiryDate }) {
    ...
    try {
      ...
      if (existing) {
        ...
        const newLink = { id:uid("sl"), partyId:shareParty.id, partyName:shareParty.name, slug, password, status:"Active", expiryDate:expiryDate||"", createdDate:today() };
        await appendRow("SharedLedgers", sharedLedgerToRow(newLink));
        setSharedLinks(prev => prev.find(l=>l.id===newLink.id) ? prev : [...prev, newLink]);
      }
      setSyncStatus("saved"); setTimeout(()=>setSyncStatus(null),2500);
      setShowShareModal(false);
    } catch(err) { setSyncStatus("error"); console.error(err); }
      ...
      <Card>
        <Table columns={[
          ...
          {key:"id", label:"Actions", render:(_,row)=>{
            ...
            return (
              <div style={{display:"flex",gap:5}}>
                {linkedBill && <Btn size="sm" variant="info" onClick={()=>setViewingBill(linkedBill)}>📄 Bill</Btn>}
                <Btn size="sm" variant="edit" onClick={()=>openEdit(row)}>✏️</Btn>
                <Btn size="sm" variant="del"  onClick={()=>setConfirmDel(row)}>🗑️</Btn>
              </div>
            );
  ...
  async function save() {
    ...
    try {
      if(editingEntry) {
        ...
        const entry={id:uid("c"),...form,amount:Number(form.amount)};
        await appendRow("Cash", cashToRow(entry));
        setCash(prev=>prev.find(c=>c.id===entry.id)?prev:[...prev,entry]);
      }
      setSyncStatus("saved"); setTimeout(()=>setSyncStatus(null),2500);
      setShowForm(false); setEditingEntry(null); setForm(blank());
    } catch(err) { setSyncStatus("error"); console.error(err); }
...
function PartyMasterScreen({ parties, setParties, ledger, setLedger, bills, setSyncStatus }) {
  ...
  async function save() {
    ...
    try {
      if(editingParty) {
        ...
        const party={id:uid("p"),...form,creditLimit:Number(form.creditLimit),openingBalance:Number(form.openingBalance)};
        await appendRow("Parties", partyToRow(party));
        setParties(prev=>prev.find(p=>p.id===party.id)?prev:[...prev,party]);
      }
      setSyncStatus("saved"); setTimeout(()=>setSyncStatus(null),2500);
      setShowForm(false); setEditingParty(null); setForm(blank());
    } catch(err) { setSyncStatus("error"); console.error(err); }
  ...
  async function handleQuickPayment({ amount, date, ref, note }) {
    ...
    try {
      const entry = {
        id: uid("l"), partyId: payParty.id, partyName: payParty.name,
        date, type: "credit", ref: ref || uid("REC"),
        note: note || "Payment received", amount,
      };
      await appendRow("Ledger", ledgerToRow(entry));
      setLedger(prev => prev.find(e=>e.id===entry.id) ? prev : [...prev, entry]);
      setSyncStatus("saved"); setTimeout(()=>setSyncStatus(null),2500);
    ...
    <div>
      ...
      <Card>
        <Table columns={[
          ...
          {key:"id", label:"Actions", render:(_,row)=>(
            <div style={{display:"flex",gap:5,flexWrap:"nowrap"}}>
              <button onClick={()=>setPayParty(row)}
                style={{background:"#EAF3DE",color:"#3B6D11",border:"1px solid #C3E8C3",borderRadius:7,padding:"5px 10px",fontSize:12,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}>
                💵 Payment
              </button>
              <Btn size="sm" variant="edit" onClick={()=>openEdit(row)}>✏️ Edit</Btn>
              <Btn size="sm" variant="del"  onClick={()=>setConfirmDel(row)}>🗑️ Del</Btn>
      ...
      {payParty && (
        <QuickPaymentModal party={payParty} currentBalance={payParty._balance} onClose={()=>setPayParty(null)} onSave={handleQuickPayment} saving={paySaving} />
      )}
      {showPrint && <PartiesPrintModal parties={parties} ledger={ledger} onClose={()=>setShowPrint(false)} />}
    </div>
  );
}
...
// ══════════════════════════════════════════════════════════════
//  ADMIN APP  ← ALL SYNC FIXES ARE HERE
// ══════════════════════════════════════════════════════════════
function AdminApp() {
  const [screen,       setScreen]       = useState("dashboard");
  const [sideOpen,     setSideOpen]     = useState(true);
  ...
  // ── loadAll: fetch fresh from server, dedupe by id so no duplicates ───────
  const loadAll = useCallback(async (silent = false) => {
    if (!silent) { setLoading(true); setLoadError(null); }
    try {
      const [pRows, iRows, bRows, lRows, cRows, slRows] = await Promise.all([
      ...
      // Deduplicate by id — server data wins; eliminates any optimistic dupes
      ...
      setParties(    dedupeById((pRows  || []).map(rowToParty)));
      setItems(      dedupeById((iRows  || []).map(rowToItem)));
      setBills(      dedupeById((bRows  || []).map(rowToBill)));
      setLedger(     dedupeById((lRows  || []).map(rowToLedger)));
      setCash(       dedupeById((cRows  || []).map(rowToCash)));
      setSharedLinks(dedupeById((slRows || []).map(rowToSharedLedger)));
    } catch (err) {
      console.error("Load error:", err);
      if (!silent) setLoadError(err.message || "Failed to load data.");
    }
  ...
  // ── runSync: flush queue → wait for Sheets to commit → reload fresh ───────
  const runSync = useCallback(async () => {
    if (getQueueLength() === 0) return;
    if (isSyncing.current) return;            // block concurrent syncs
    ...
    try {
      ...
      if (failed > 0) {
        ...
        // Give Google Sheets 800 ms to finish committing before we read back
        await new Promise(resolve => setTimeout(resolve, 800));

        // Full reload — replaces ALL local state with server truth.
        // dedupeById inside loadAll ensures no duplicates even on retry.
        await loadAll(true);
